    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result != null) {
                ...
                var Data =
                    '<h3 class="card-title d-block" style="font-size:13px;">Mã Oder: <small>' + result.order + '</small></h3>' +
                ...
                for (let i = 0; i < result.listdata.length; i++) {
                    ...
                    if (result.listdata[i].moneyy > 0) {
                        var data =
                            '<tr>' +
                            '<td class="text-center text-danger">' + j + '</td>' +
